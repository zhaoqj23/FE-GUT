...
figure;
% 逐个添加图例项
labelVarwin1 = 'GSSM with window size ' + string(15); % 假设value是一个你想在图例中显示的变量
labelVarwin2 = 'GSSM with window size ' + string(20); % 假设value是一个你想在图例中显示的变量
labelVarwin3 = 'GSSM with window size ' + string(25); % 假设value是一个你想在图例中显示的变量
labelVarwin4 = 'GSSM with window size '+ string(30); % 假设value是一个你想在图例中显示的变量
...
plot(time(winsize(4):N-1),M_opt(4,winsize(4):N-1,1),'r-','LineWidth',2);
hold;
set(gca, 'FontSize', 24);
ylabel('Nonlinearity','fontsize',24);xlabel('Time(s)','fontsize',24);
%box off;
grid on;
legend(labelVarwin1,labelVarwin2,labelVarwin3,labelVarwin4,'FontSize', 20);%
% 获取当前图例的句柄
lgd = legend;
% 获取图例的当前位置和大小
pos = get(lgd, 'Position');
% 设置图例框的大小（根据需要调整数值）
pos(3) = pos(3) * 1.0; % 增加宽度为原来的1.5倍
pos(4) = pos(4) * 1.0; % 增加高度为原来的1.5倍
% 应用新的位置和大小
