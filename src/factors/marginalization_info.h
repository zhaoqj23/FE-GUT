...
public:
    ...
    void addResidualBlockInfo(const std::shared_ptr<ResidualBlockInfo> &blockinfo) {
        ...
        for (size_t k = 0; k < parameter_blocks.size(); k++) {
            ...
            //是和待边缘化的变量相关的变量的大小和地址，键是地址，值是大小
        }
        ...
        for (int index : blockinfo->marginalizationParametersIndex()) {
            ...
            //parameter_block_index_存储的是要边缘化的变量的地址，键为地址，值为0（值应该是没有意义的
        }
    }
...
private:
    ...
    bool updateParameterBlocksIndex() {
        int index = 0;
        // 只有被边缘化的参数预先加入了表
        // parameter_block_index_待边缘化的变量的地址和索引
        // parameter_block_size_所有的变量的地址和大小
        for (auto &block : parameter_block_index_) {
            block.second = index;
            index += parameter_block_size_[block.first];
        }
        marginalized_size_ = index;//这里待边缘化的参数大小是11
    ...
    void preMarginalization() {
        for (const auto &factor : factors_) {
            ...
            for (size_t k = 0; k < block_sizes.size(); k++) {
                ...
                // 拷贝参数块数据
                // parameter_block_data_原始参数块数据的copy
